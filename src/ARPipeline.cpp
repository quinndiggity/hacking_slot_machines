...
bool ARPipeline::processFrame(const cv::Mat& inputFrame, cv::Mat& m_warpedImg) {
	bool patternFound = m_patternDetector.findPattern(inputFrame, m_warpedImg, m_patternInfo);
	if (patternFound) {m_patternInfo.computePose(m_pattern, m_calibration);}
	return patternFound;
}
